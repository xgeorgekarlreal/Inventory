...
const RecordSaleModal: React.FC<RecordSaleModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const result = await InventoryService.recordSale(formData)
      if (result.success) {
        onSuccess()
        onClose()
      } else {
      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <h3 className="text-lg font-medium text-gray-900">Record Sale</h3>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                placeholder="Optional reference ID (e.g., invoice number)"
              />
            </div>
